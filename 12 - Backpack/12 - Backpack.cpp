...
//Рекурсивная функция для неограниченного рюкзака
...
int main() {
    ...
    return 0;
        ...
        // Идем с конца, чтобы не использовать один и тот же предмет дважды за один проход

            // Для каждой возможной вместимости w, у нас есть два варианта:
            // 1. Не брать текущий предмет. Тогда ценность для веса w останется прежней (dp[w]).
            // 2. Взять текущий предмет (если он помещается).
            //    Тогда его ценность item.value добавляется к максимальной ценности,
            //    которую можно было набрать с оставшимся весом (w - item.weight).
            // Выбираем вариант, который дает большую ценность.
